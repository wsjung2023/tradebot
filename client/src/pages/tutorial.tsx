...
const steps: TutorialStep[] = [
  ...
  },
  {
    ...
    operatorAction: [
      "자동매매 메뉴 상단에 뱃지(숫자)가 표시되면 학습 잡이 파라미터 개선 제안을 생성한 것",
      "자동매매 > 학습 파라미터 제안 패널에서 '익절%를 X→Y로 변경' 같은 제안을 검토",
      "제안 카드의 '적용' 버튼으로 개별 반영하거나, '전체 무시'로 이번 사이클 건너뜀",
      "설정 > 학습 파라미터 적용 방식 토글로 '자동 반영' 모드로 전환 가능",
    ],
    systemAction: [
      "모델 기준으로 과거 보관 계좌 + 현재 활성 계좌의 매매 로그를 함께 분석 (최소 20건)",
      "성과가 좋은 지표의 가중치를 높이고, 나쁜 지표는 낮추는 최적화 제안을 DB에 pending 상태로 저장",
      "사용자가 '적용' 클릭 시 해당 파라미터를 자동으로 모델 설정에 반영",
    ],
    checkpoints: [
      "자동매매 메뉴에 파란색 뱃지가 표시되는가?",
      "학습 파라미터 제안 패널에서 제안 카드가 나타나는가?",
