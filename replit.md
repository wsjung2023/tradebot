...
## Agent Communication Rules (절대 규칙)
- 집 PC 에이전트는 절대 꺼져 있을 리 없으므로 "에이전트가 꺼져 있다", "PC가 꺼져 있다" 같은 말 절대 금지
- 에러 메시지 변경, UI 문구 수정 같은 임기웅변 짓거리 금지 — 근본 원인을 코드로 해결할 것
- 사용자에게 보내는 메시지로 변명하거나 설명하는 것 금지 — 동작하는 코드로 보여줄 것
...
7.  **에이전트 알림**: 집 PC 에이전트 연결 끊김/복구 시 이메일 알림 (nodemailer + SMTP). 설정 → "에이전트 연결 끊김 알림" 카드에서 이메일/임계값 설정.
